class RC5:
    ...
    def __init__(self, w, R, key, strip_extra_nulls=False):
        self.w = w  # размер блока (32, 64 или 128 бит)
        self.R = R  # номер круга (от 0 до 255)
        self.key = key  # ключ (от 0 до 2040 бит)
    ...
    def __const(self):  # функция генерации констант
        if self.w == 16:
            return 0xB7E1, 0x9E37  # return P или Q
        elif self.w == 32:
    ...
    def __keyAlign(self):
        if self.b == 0:  # ключ путой
            self.c = 1
        elif self.b % self.w8: # ключ не кратен w / 8
            self.key += b'\x00' * (self.w8 - self.b % self.w8)  # дополнить ключ \x00 (пустыми) байтаим
            self.b = len(self.key)
            self.c = self.b // self.w8
        else:
            self.c = self.b // self.w8
        L = [0] * self.c
        for i in range(self.b - 1, -1, -1): # заполняем список L
            L[i // self.w8] = (L[i // self.w8] << 8) + self.key[i]
        self.L = L

    def __keyExtend(self): # заполняем список S
        P, Q = self.__const()
